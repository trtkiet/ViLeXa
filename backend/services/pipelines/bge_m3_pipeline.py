"""BGE-M3 RAG Pipeline using Qdrant vector store with configurable retrieval modes."""
...
class BGEM3Pipeline(RAGPipeline):
    """
    RAG pipeline using BAAI/bge-m3 multilingual embeddings with Qdrant.
    ...
    Args:
        ...
        collection_name: Qdrant collection (defaults to config.BGEM3_COLLECTION_NAME).
        qdrant_host: Qdrant host (defaults to config.QDRANT_HOST).
    ...
    def __init__(
        ...
        Args:
            ...
            reranker: Optional custom reranker instance (takes precedence over use_reranker).
            collection_name: Qdrant collection name (defaults to config.BGEM3_COLLECTION_NAME).
            qdrant_host: Qdrant host (defaults to config.QDRANT_HOST).
            qdrant_port: Qdrant port (defaults to config.QDRANT_PORT).
            skip_llm: Skip LLM initialization (for retrieval-only benchmarks).
            retrieval_k: Number of candidates to retrieve (defaults to config.RETRIEVAL_K).
    ...
    def startup(self) -> None:
        ...
        logger.info("--- Starting BGEM3Pipeline Initialization ---")
        t_start = time.time()

        # Resolve configuration with defaults
        retrieval_mode = self.retrieval_mode or config.RETRIEVAL_MODE
        collection_name = self.collection_name or config.BGEM3_COLLECTION_NAME
        ...
        # 2. Initialize BGE-M3 Embedding (only load what's needed for the mode)
        bge_m3_engine = BGEM3Embedding(
            model_name=config.BGEM3_EMBEDDING_MODEL,
            device="cpu",
        )
        ...
        if retrieval_mode in ("hybrid", "dense"):
            dense_embeddings = BGEM3DenseAdapter(bge_m3_engine)
            logger.info("Dense embeddings enabled")

        if retrieval_mode in ("hybrid", "sparse"):
            sparse_embeddings = BGEM3SparseAdapter(bge_m3_engine)
        ...
        logger.info(
            f"--- BGEM3Pipeline Initialization Complete in {time.time() - t_start:.2f}s ---"
        )
